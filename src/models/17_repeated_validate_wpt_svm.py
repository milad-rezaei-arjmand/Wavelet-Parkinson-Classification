...
RESULTS_DIR = (
    OUTPUTS_DIR / "repeated_wpt_validation"
)
...
def load_dataset() -> tuple[
    ...
    if not WPT_FEATURE_PATH.exists():
        raise FileNotFoundError(
            f"WPT feature file not found: {WPT_FEATURE_PATH}"
        )

    print("Loading WPT features...")

    data = pd.read_parquet(WPT_FEATURE_PATH)
    ...
    if len(feature_columns) != 1728:
        raise ValueError(
            f"Expected 1728 WPT features, found {len(feature_columns)}."
        )
...
def main() -> None:
    ...
    print("\n" + "=" * 88)
    print("REPEATED NESTED SUBJECT-GROUPED VALIDATION — WPT SVM")
    print("=" * 88)
    ...
    print(f"WPT features: {X.shape[1]}")
