...
FEATURE_OUTPUT_PATH = (
    OUTPUT_DIR / "swt_features_db4_level5.parquet"
)

FEATURE_MANIFEST_PATH = (
    OUTPUT_DIR / "swt_feature_manifest.csv"
)

ERROR_OUTPUT_PATH = (
    OUTPUT_DIR / "swt_extraction_errors.csv"
)
...
# With trim_approx=True, pywt.swt returns:
# [A5, D5, D4, D3, D2, D1]
BAND_NAMES = [
    "A5",
    "D5",
...
def safe_kurtosis(
    ...
    Normal-distribution kurtosis is approximately 3.
    """
...
def decompose_signal_swt(
    ...
    if signal.ndim != 1:
        raise ValueError(
            "Expected a one-dimensional signal, "
    ...
    if (
        ...
        raise ValueError(
            "Signal length must be divisible by "
            f"2^{SWT_LEVEL} for SWT."
        )

    if not np.all(np.isfinite(signal)):
        raise ValueError(
            "SWT input contains NaN or Inf."
        )
    ...
    coefficient_arrays = [
        ...
        for coefficients_array
        in coefficients
    ]

    if len(coefficient_arrays) != len(BAND_NAMES):
        raise RuntimeError(
            "Unexpected SWT band count: "
            f"{len(coefficient_arrays)} "
            f"instead of {len(BAND_NAMES)}."
        )
    ...
    for (
        ...
        if coefficients_array.shape != (
            EXPECTED_WINDOW_SIZE,
        ):
            raise RuntimeError(
                f"Unexpected shape for {band_name}: "
                f"{coefficients_array.shape}"
            )
        ...
        if not np.all(
            ...
            raise ValueError(
                f"{band_name} contains NaN or Inf."
            )

    return coefficient_arrays
...
def extract_paired_window_features(
    ...
    if len(features) != len(
        ...
        raise RuntimeError(
            "Unexpected total SWT feature count: "
    ...
    if not np.all(
        ...
        raise ValueError(
            "Extracted SWT features contain "
            "NaN or Inf."
        )
...
def create_feature_manifest() -> pd.DataFrame:
    ...
    descriptions = {
        "mean": (
            "Mean of SWT coefficients"
        ),
        "variance": (
            "Variance of SWT coefficients"
        ),
        "std": (
            "Standard deviation of SWT coefficients"
        ),
        "energy": (
            "Sum of squared SWT coefficients"
        ),
        "relative_energy": (
            "Band energy divided by total "
            "channel SWT-band energy"
        ),
        "entropy": (
            "Shannon entropy of normalized "
            "squared SWT coefficients"
        ),
        "iqr": (
                    ...
                    rows.append(
                        {
                            ...
                            "boundary_note": (
                                "PyWavelets SWT requires "
                                "length divisible by 2^level "
                                "and uses an undecimated "
                                "periodic/circular formulation."
                            ),
                        }
                    )
...
def main() -> None:
    ...
    create_feature_manifest().to_csv(
        FEATURE_MANIFEST_PATH,
        index=False,
        encoding="utf-8-sig",
    )
    ...
    print("\n" + "=" * 76)
    print("SWT FEATURE EXTRACTION SUMMARY")
    print("=" * 76)

    print(f"\nWavelet: {WAVELET_NAME}")
    print(f"SWT level: {SWT_LEVEL}")
    print("trim_approx: True")
    print("norm: True")
    print(
        "Coefficient length per band: "
        f"{EXPECTED_WINDOW_SIZE}"
    )
    ...
    if processed_rows != len(
        ...
        raise RuntimeError(
            "Extracted row count does not "
            "match the window inventory. "
